...
export default function TeacherLoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // Store the teacher role in localStorage before initiating the OAuth flow
      localStorage.setItem("selectedUserRole", "teacher");

      await signIn("google", {
        callbackUrl: "/login/anbieter", // Redirect back to teacher login to handle role setting
      });
    } catch {
      setError(
      ...
      <div className="max-w-4xl w-full flex bg-white shadow-md rounded-lg overflow-hidden">
        <div className="w-1/2 flex flex-col items-center justify-center bg-gray-100 p-8">
          ...
          <p className="text-gray-600 mt-4 text-center">
            Anmeldung für Kursanbieter
          </p>
        </div>
        <div className="w-1/2 p-8">
          <h2 className="text-3xl font-bold text-gray-900 mb-4">Anmelden als Anbieter</h2>
          <div className="space-y-4">
            ...
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-800">
                <strong>Sie melden sich als Kursanbieter an.</strong> Als Anbieter können Sie Kurse erstellen und verwalten.
              </p>
            </div>
