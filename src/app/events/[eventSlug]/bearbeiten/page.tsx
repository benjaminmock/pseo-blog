...
async function getEventBySlug(slug: string, trainerId: number) {
  const stmt = db.prepare(`
    SELECT
      e.event_id,
      e.event_name,
    ...
    WHERE e.slug = ? AND e.trainer_id = ?
  `);

  return stmt.get(slug, trainerId) as
    | {
        event_id: number;
        event_name: string;
        description: string;
...
export default async function EditEventPage({
  ...
  const event = await getEventBySlug(params.eventSlug, trainerId);
  if (!event) {
    notFound();
  }
